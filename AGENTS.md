...
- Use subagents when the user explicitly allows delegation or asks for parallel agent work.
- Keep critical-path decisions, cross-cutting edits, final integration, and final verification in the main agent.
- Delegate only independent sidecar work such as codebase exploration, targeted test discovery, or bounded code changes with a disjoint write scope.
- Prefer `explorer` subagents for read-only codebase questions and `worker` subagents for bounded implementation work.
- Give each worker an explicit file or module ownership boundary and do not assign overlapping write scopes in parallel.
- After delegating, continue non-overlapping local work instead of waiting unless the next step is blocked on the delegated result.
- For mixed Flutter and Spring Boot tasks, split subagents by stack boundary rather than mixing both stacks in one delegated task.
...
- Prefer parallel execution for independent verification commands to reduce wall-clock time.
- Split verification into phases:
  - Phase 1: run setup or generation commands that write files sequentially.
  - Phase 2: run independent read-only verification commands in parallel.
- Do not parallelize commands that write the same outputs, depend on files being generated by another running command, or are likely to fight over the same generated-code state.
...
Run regeneration commands only if their trigger applies, in sequence. After generation is complete, run these in parallel:
...
- When multiple selected verification commands are read-only and independent, run them in parallel instead of sequentially.
...
- If backend verification expands to multiple independent read-only commands, prefer parallel execution after any required setup or generation steps complete.
...
- when delegation is useful, keep Flutter and Spring Boot write scopes on separate subagents and integrate in the main agent
