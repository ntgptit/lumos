...
public class DeckServiceImpl implements DeckService {
    ...
    public DeckResponse createDeck(Long folderId, CreateDeckRequest request) {
        ...
        this
                .validateFolderCanCreateDeck(folder
                        .getId());
        ...
        final var normalizedDescription = this
                .normalizeDescription(request
                        .description());
        this
                .validateDeckName(folder
                        .getId(), normalizedName, null);

        final var deck = this.deckMapper
                .toDeckEntity(
                        folder,
                        normalizedName,
                        normalizedDescription,
                        DeckConstants.FLASHCARD_COUNT_DEFAULT);
        final var savedDeck = this.deckRepository
                .save(deck);
        // Return the freshly created deck DTO after persistence has assigned its
        // canonical state.
        return this.deckMapper
                .toDeckResponse(savedDeck);
    }
    ...
    public DeckResponse updateDeck(Long folderId, Long deckId, UpdateDeckRequest request) {
        final var deck = this
                .findActiveDeck(folderId, deckId);
        ...
        final var normalizedDescription = this
                .normalizeDescription(request
                        .description());
        this
                .validateDeckName(folderId, normalizedName, deck
                        .getId());
        ...
        // Return the updated deck DTO so the client sees normalized values from the
        // managed entity.
        return this.deckMapper
                .toDeckResponse(deck);
    }
    ...
    public void deleteDeck(Long folderId, Long deckId) {
        this
                .findActiveDeck(folderId, deckId);
        this.deckRepository
                .softDeleteDeck(folderId, deckId, Instant
                        .now());
    }
    ...
    public List<DeckResponse> getDecks(Long folderId, SearchRequest searchRequest, Pageable pageable) {
        ...
        final var specification = DeckSpecifications
                .byFolderAndKeyword(folderId, searchRequest
                        .searchQuery());
        final var sortedPageable = DeckSpecifications
                .toSortedPageable(pageable, searchRequest
                        .sortBy(),
                        searchRequest
                                .sortType());
        final var page = this.deckRepository
                .findAll(specification, sortedPageable);

        // Convert page entities to API DTOs before returning them to the folder detail
        // screen.
        return page
                .getContent()
                .stream()
                .map(this.deckMapper::toDeckResponse)
                .toList();
    }
    ...
    public DeckImportResponse importDecks(Long folderId, MultipartFile file) {
        ...
        this
                .validateFolderCanCreateDeck(folder
                        .getId());
        final var deckDrafts = this.deckExcelImportSupport
                .parseExcelFile(file);
        final var importTimestamp = Instant
                .now();
        final var createdDeckCount = this.deckImportBatchRepository
                .insertDecksIgnoreConflicts(folderId, deckDrafts, importTimestamp);
        final var deckByNormalizedName = this
                .resolveDeckByNormalizedName(folderId, deckDrafts);
        final var flashcardCommands = this
                .toFlashcardImportBatchCommands(deckDrafts, deckByNormalizedName);
        final var importedFlashcardCount = this.flashcardImportBatchRepository
                .insertFlashcards(flashcardCommands, importTimestamp);
        final var flashcardCountByDeckId = this
                .resolveFlashcardCountByDeckId(flashcardCommands);
        this.deckImportBatchRepository
                .incrementFlashcardCounts(flashcardCountByDeckId, importTimestamp);
        // Return the import summary so the client can refresh deck and flashcard counts
        // together.
        return this.deckMapper
                .toDeckImportResponse(
                        folderId,
                        deckDrafts
                                .size(),
                        createdDeckCount,
    ...
    private Folder findActiveFolder(Long folderId) {
        // Return the active folder or fail so deck operations never run against deleted
        // or missing folders.
    ...
    private Deck findActiveDeck(Long folderId, Long deckId) {
        final var deck = this.deckRepository
                .findByIdAndDeletedAtIsNull(deckId)
                .orElseThrow(() -> new DeckNotFoundException(deckId));
        final var deckFolderId = deck
                .getFolder()
                .getId();
        // Ensure deck belongs to the requested folder scope.
        if (deckFolderId != null && folderId != null && deckFolderId
                .longValue() == folderId
                        .longValue()) {
            // Return the deck only when the nested folder path matches the deck ownership.
            return deck;
        }
        // Hide decks outside the requested folder scope so callers cannot probe foreign
        // deck ids.
        throw new DeckNotFoundException(deckId);
    }

    private void validateFolderCanCreateDeck(Long folderId) {
        final var hasChildFolders = this.folderRepository
                .existsByParentIdAndDeletedAtIsNull(folderId);
        ...
        // Prevent decks from being created in non-leaf folders because folders cannot
        // mix decks and subfolders.
    ...
    private void validateDeckName(Long folderId, String deckName, Long excludeDeckId) {
        final var exists = this.deckRepository
                .existsActiveNameByFolderId(folderId, deckName, excludeDeckId);
        ...
        // Reject duplicate deck names inside the same folder to keep the deck list
        // unambiguous.
    ...
    private String normalizeDescription(String description) {
        ...
        // Return the trimmed description so storage does not preserve accidental outer
        // whitespace.
        return StringUtils
                .trim(description);
    }
    ...
    private String normalizeLookupKey(String value) {
        // Return the lower-case lookup key so deck-name conflict checks stay
        // case-insensitive.
        return StringUtils
                .lowerCase(value);
    }
    ...
    private Map<String, Deck> resolveDeckByNormalizedName(Long folderId, List<DeckImportDeckDraft> deckDrafts) {
        ...
        // Collect normalized deck names so the follow-up lookup resolves both new and
        // existing decks.
        for (DeckImportDeckDraft deckDraft : deckDrafts) {
            normalizedNames
                    .add(this
                            .normalizeLookupKey(deckDraft
                                    .name()));
        }
        final var decks = this.deckRepository
                .findAllActiveByFolderIdAndNormalizedNames(folderId, normalizedNames);
        final Map<String, Deck> deckByNormalizedName = new LinkedHashMap<>();
        // Re-index resolved deck entities by normalized name so flashcard rows can
        // attach to deck ids quickly.
        for (Deck deck : decks) {
            deckByNormalizedName
                    .put(this
                            .normalizeLookupKey(deck
                                    .getName()),
                            deck);
        }
        // Return the resolved deck map when every imported deck name exists after the
        // insert-or-ignore step.
        if (deckByNormalizedName
                .size() == deckDrafts
                        .size()) {
            // Return the resolved deck map so downstream flashcard batching can use stable
            // deck ids.
            return deckByNormalizedName;
        }
        // Stop immediately because flashcard rows cannot be assigned when any deck id
        // is unresolved.
    ...
    private List<FlashcardImportBatchCommand> toFlashcardImportBatchCommands(
        ...
        // Walk each imported deck section to turn grouped flashcard rows into
        // batch-insert commands.
        for (DeckImportDeckDraft deckDraft : deckDrafts) {
            final var normalizedDeckName = this
                    .normalizeLookupKey(deckDraft
                            .name());
            final var deck = deckByNormalizedName
                    .get(normalizedDeckName);
            // Stop immediately because every flashcard row must resolve to a persisted deck
            // id.
            if (deck == null) {
                // Reject the import because the deck resolution step did not yield a deck id
                // for this section.
                throw new DeckImportFileInvalidException(ErrorMessageKeys.DECK_IMPORT_READ_FAILED);
            }
            this
                    .addFlashcardImportCommands(flashcardCommands, deck, deckDraft);
        }
        // Return the flattened flashcard batch commands so JDBC batching can persist
        // them efficiently.
    ...
    private Map<Long, Integer> resolveFlashcardCountByDeckId(List<FlashcardImportBatchCommand> flashcardCommands) {
        ...
        // Aggregate imported flashcard counts by deck id so deck counters update once
        // per deck.
        for (FlashcardImportBatchCommand flashcardCommand : flashcardCommands) {
            final var currentCount = flashcardCountByDeckId
                    .getOrDefault(
                            flashcardCommand
                                    .deckId(),
                            DeckConstants.FLASHCARD_COUNT_DEFAULT);
            flashcardCountByDeckId
                    .put(flashcardCommand
                            .deckId(), currentCount + 1);
        }
        // Return the aggregated deck counts so deck counter updates stay batched and
        // deterministic.
    ...
    private void addFlashcardImportCommands(
        ...
        // Append every flashcard row in the current deck section as one JDBC batch
        // command.
        for (var flashcardDraft : deckDraft
                .flashcards()) {
            flashcardCommands
                    .add(new FlashcardImportBatchCommand(
                            deck
                                    .getId(),
                            flashcardDraft
                                    .term(),
                            flashcardDraft
                                    .meaning(),
                            DeckImportConstants.TERM_LANGUAGE_CODE_DEFAULT
                                    .name(),
                            DeckImportConstants.MEANING_LANGUAGE_CODE_DEFAULT
                                    .name(),
