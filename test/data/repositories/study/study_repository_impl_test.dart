...
void main() {
  group('DioStudyRepository', () {
    test('calls study endpoints and maps payloads', () async {
      ...
      dio.interceptors.add(
        InterceptorsWrapper(
          onRequest: (RequestOptions options, RequestInterceptorHandler handler) {
            if (options.path == StudyRepositoryImplConst.remindersSummaryPath) {
              handler.resolve(
                Response<dynamic>(
            ...
            if (options.path == StudyRepositoryImplConst.analyticsOverviewPath) {
              handler.resolve(
                Response<dynamic>(
                  requestOptions: options,
                  data: <String, dynamic>{
            ...
            if (options.path == StudyRepositoryImplConst.speechPreferencePath) {
              handler.resolve(
                Response<dynamic>(
                  requestOptions: options,
                  data: <String, dynamic>{
      ...
      final StudySessionData started = await repository.startSession(deckId: 10);
      final StudySessionData resumed = await repository.resumeSession(sessionId: 33);
      final StudySessionData submitted = await repository.submitAnswer(
        sessionId: 33,
        answer: 'xin chao',
      );
      final StudySessionData revealed = await repository.revealAnswer(sessionId: 33);
      final StudySessionData remembered = await repository.markRemembered(
        sessionId: 33,
      );
      final StudySessionData retried = await repository.retryItem(sessionId: 33);
      ...
      final SpeechPreference preference = await repository.getSpeechPreference();
